...
class SketchDataset(torch.utils.data.Dataset):
    def __init__(self, opt, root, class_name, split='train', permutation=False):
        ...
        self.pt_dir = osp.join(root,opt.class_name, '{}_{}.pt'.format(self.class_name, self.split))
        
        self.json_dir = osp.join(root,opt.class_name, '{}_{}.ndjson'.format(self.class_name, self.split))
    ...
    def __getitem__(self, index):
        png_path=self.recog_png_paths[index].split("/")
        png_path=osp.join("./data",self.opt.class_name,"pngs",png_path[-2],png_path[-1])
    ...
    def _process(self, perm_arg=None):
        ...
        with open(self.json_dir, 'r') as f:
            for line in f:
                ...
                raw_data.append(line1["drawing"])
                recog_label_datas.append(int(line1["recog_label"]))
                recog_png_paths.append(line1["png_path"])
                key_ids.append(line1["key_id"])
