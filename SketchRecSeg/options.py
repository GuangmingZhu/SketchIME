...
class BaseOptions:
    ...
    def initialize(self):
        ...
        self.parser.add_argument('--points-num', type=int, default=300, 
    ...
    def parse(self, params=None):
        ...
        if self.is_train:
            ...
            # save to the disk
            expr_dir = os.path.join(self.opt.checkpoints_dir, self.opt.class_name, self.opt.timestamp)
            mkdir(expr_dir)
...
class TestOptions(BaseOptions):
    def initialize(self):
        ...
        self.parser.add_argument("--permutation", action='store_true',
                                 help='if data permutation')
        self.parser.add_argument("--rsm_num", type=float, default=0.99,
                                 help='if data permutation')
        self.parser.add_argument("--top_k", type=int, default=3,
                                 help='if data permutation')
        self.parser.add_argument("--M", type=float, default=0.3,
                                 help='if data permutation')
        self.is_train = False
